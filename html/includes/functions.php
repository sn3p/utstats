...
function compress_file($method, $in, $out, $stripx00)
{
  ...
  switch ($method) {
    case 'bz2':
      $fp_out = @bzopen($out, 'wb');
      break;
    case 'zlib':
      $fp_out = @gzopen($out, 'wb6');
      break;
    case 'none':
      $fp_out = @fopen($out, 'wb');
      break;
  ...
  while (!feof($fp_in)) {
    ...
    switch ($method) {
      case 'bz2':
        $bytes = @bzwrite($fp_out, $buffer, strlen($buffer));
        break;
      case 'zlib':
        $bytes = @gzwrite($fp_out, $buffer, strlen($buffer));
        break;
      case 'none':
        $bytes = @fwrite($fp_out, $buffer, strlen($buffer));
        break;
    }
...
function getMapImageName($mapname)
{
  for ($i = 0; $i < 3; $i++) {
    ...
    // try substracting league names or other known suffixes from the map
    if ($i == 2) {
      $mapVersions = array(
        'CB1',
        'CB2',
